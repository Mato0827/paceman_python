...
def print_player_details(sc):
    ...
    draw_text(sc,"COIN       :   "+str(declear_var.pl_coin),declear_var.SCREEN_SIZE+70,790,35,declear_var.BLACK,False)
    draw_text(sc,"LIFE        :   "+str(declear_var.pl_life),declear_var.SCREEN_SIZE+70,830,35,declear_var.BLACK,False)
...
def print_enemy_details(sc):
    ...
    for i in range(6):
        ...
        sc.blit(ri.img_rz,[declear_var.SCREEN_SIZE+70,50+50*i])

        draw_text(sc,"X  "+str(count_enemy_color[i]),declear_var.SCREEN_SIZE+150,60+50*i,35,declear_var.BLACK,False)
...
def print_paceman_details(sc):
    for i in range(1,6):
        ...
        draw_text(sc,"["+str(i)+"]:",declear_var.SCREEN_SIZE+50,380+50*i,35,declear_var.BLACK,False)
        draw_text(sc,"X   "+str(declear_var.pl_item[i]),declear_var.SCREEN_SIZE+180,380+50*i,35,declear_var.BLACK,False)
        ...
        for x in range(-7,8):
            ...
            if 0 <= mx < declear_var.maze_num and 0 <= my < declear_var.maze_num:
                ...
                for n in range(declear_var.emy_max):
                    if declear_var.emy_f[n] == False:
                        continue
                    if declear_var.emy_x[n] == mx and declear_var.emy_y[n] == my:
                        sc.blit(ri.img_enemy[declear_var.emy_col[n]*4 + declear_var.emy_d[n]],[X,Y])
            if x == 0 and y == 0:
                if declear_var.pl_muteki%2 == 0:
                    ...
                    if declear_var.item_use == True and declear_var.item_time<declear_var.FPS*3:
